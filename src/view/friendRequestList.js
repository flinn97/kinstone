...
class MainContent extends Component{
  ...
  render(){
    ...
    <div style={{display: "flex", flexDirection: "row"}}>
      <MapComponent app={app} name="friendRequest" cells={[ "owner", {txt:"accept!"} ]} functions={{
            cells: [1], functions: [async (component) => {
              debugger
              //our friend object
              await opps.cleanJsonPrepareRun({ addfriend: {email: component.getJson().owner, type: "friend", _id: Math.random(Math.floor()*100000).toString() } })
              //their friend object 
              await opps.cleanJsonPrepareRun({ addfriend: {owner: component.getJson().owner, email: state.user.getJson().owner ,type: "friend", _id: Math.random(Math.floor()*100000).toString() } })

              await opps.cleanPrepareRun({del: component})
          }]}}
 />
    </div>
    )
